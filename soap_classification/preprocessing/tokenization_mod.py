...
class WordpieceTokenizer(object):
    ...
    def tokenize(self, text):
        ...
        for token in whitespace_tokenize(text):
            ...
            # 1トークンの文字数がmaxを超えていたら[UNK]として返す
...
#################################
# Appended MecabTokenizer Class #
#################################
class MecabTokenizer(BasicTokenizer):

    def __init__(
        self,
        mecab_ipadic_neologd,
        mecab_J_medic,
        anonymize_person_name=False,
        name_token="＠＠Ｎ",
    ):
        """Constructs a BasicTokenizer."""
        import MeCab
    ...
    def tokenize(self, text):
        ...
        try:
            self._mecab.parse("")  # Prevent strings from being Garbage collection
...
########################################
# Appended FullTokenizerForMecab Class #
########################################
class FullTokenizerForMecab(object):
    """Runs end-to-end tokenziation."""

    def __init__(self, sub_tokenizer, vocab_file, do_lower_case=False):
        self.vocab = load_vocab(vocab_file)
        self.inv_vocab = {v: k for k, v in self.vocab.items()}
    ...
    def tokenize(self, text):
        split_tokens = []
        for token in self.sub_tokenizer.tokenize(text):
            for sub_token in self.wordpiece_tokenizer.tokenize(token):
                split_tokens.append(sub_token)
