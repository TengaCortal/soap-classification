...
x_sections_df = pd.read_csv(os.path.join(SOAPS_PATH, "x_sections.csv"))
...
with open(
    os.path.join(os.path.join(BASE_PATH, "resources/soaps"), "labeled_dataset.csv"), "w"
) as output_file:
    output_file.write("SOAP,label\n")  # Write header

    # Iterate through each dataframe and write rows to the new CSV file
    for df in [
        # x_sections_df.sample(n=3000, random_state=42),
        # sub_sections_df.sample(n=11000, random_state=42),
        # obj_sections_df.sample(n=3000, random_state=42),
        # asm_sections_df.sample(n=9000, random_state=42),
        # pls_sections_df.sample(n=3000, random_state=42),
        x_sections_df,
        sub_sections_df,
        obj_sections_df,
        asm_sections_df,
...
labeled_dataset_df = pd.read_csv(
    os.path.join(os.path.join(BASE_PATH, "resources/soaps"), "labeled_dataset.csv")
)
